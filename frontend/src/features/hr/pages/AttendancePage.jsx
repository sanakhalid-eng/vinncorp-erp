...
export default function AttendancePage() {
        ...
        <div className="mt-8 bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                {['Employee', 'Shift', 'Check In', 'Check Out', 'Hours', 'Status', 'Actions'].map((h) => (
  ...
  if (error) return <ErrorState title="Failed to load attendance" message={error} onRetry={load} />;
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      {filtered.length === 0 ? (
        <EmptyState title="No attendance records" message={`No records found for ${date}.`} />
      ) : (
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-700">{r.shiftName || 'ΓÇö'}</td>
                  <td className="px-4 py-3 text-slate-700">
                    {r.checkInTime ? new Date(r.checkInTime).toLocaleTimeString() : 'ΓÇö'}
                  </td>
                  <td className="px-4 py-3 text-slate-700">
                    {r.checkOutTime ? new Date(r.checkOutTime).toLocaleTimeString() : 'ΓÇö'}
                  </td>
                  <td className="px-4 py-3 text-slate-700">{r.workHours ?? 'ΓÇö'}</td>
                  <td className="px-4 py-3">
