...
export default function UtilizationReportPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (empData.status === 'fulfilled') setDepartments([]);
      if (deptData.status === 'fulfilled') setDepartments(deptData.value);
      if (utilData.status === 'rejected') setError(utilData.reason?.message || 'Failed to load utilization data');
    } catch (e) {
      setError(e.message || 'Failed to load utilization data');
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                {['Employee', 'Department', 'Logged / Expected', 'Utilization', 'Overtime', 'Attendance', 'Rating'].map((h) => (
  ...
  if (error) {
    return <ErrorState title="Failed to load utilization data" message={error} onRetry={loadData} />;
  }
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      {filtered.length === 0 ? (
        <EmptyState title="No data found" message="No utilization data matches your filters." />
      ) : (
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((emp) => (
                <tr key={emp.employeeId} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-700">{emp.department || 'ΓÇö'}</td>
