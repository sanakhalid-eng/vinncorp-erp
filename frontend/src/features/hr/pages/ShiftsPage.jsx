...
export default function ShiftsPage() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteShift(itemToDelete.id);
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          icon={Clock}
          title="No shifts yet"
          description="Create work shifts to define start times, breaks, and grace periods."
          action={{ label: 'New Shift', icon: Plus, onClick: openCreate }}
        />
      ) : (
