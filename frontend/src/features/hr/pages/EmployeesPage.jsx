...
import EmptyStateWithCta from '../../../components/EmptyState';
    ...
    <div className="p-4 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by name, code, emailΓÇª"
      ...
      {filtered.length === 0 ? (
        <EmptyStateWithCta
          ...
          description={search || statusFilter || deptFilter ? "Try adjusting your filters." : "Add your first employee to get started."}
          action={!search && !statusFilter && !deptFilter ? { label: 'New Employee', icon: Plus, onClick: openCreate } : undefined}
        />
      ) : (
              ...
              {filtered.map((e) => (
                <tr key={e.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3">
                    ...
                    <div className="text-slate-500 text-xs">{e.workEmail || 'ΓÇö'}</div>
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-slate-700">{e.employeeCode}</td>
                  <td className="px-4 py-3 text-slate-700">
                    {e.departmentId ? deptMap[e.departmentId]?.name : 'ΓÇö'}
                  </td>
                  <td className="px-4 py-3 text-slate-700">
                    {e.designationId ? desigMap[e.designationId]?.title : 'ΓÇö'}
                  </td>
