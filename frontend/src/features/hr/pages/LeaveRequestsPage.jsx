...
import { FileText, Plus, Search, XCircle, Eye } from 'lucide-react';
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-700 truncate max-w-xs">{r.reason || 'ΓÇö'}</td>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setViewing(null)}>
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div><span className="font-medium text-slate-600">Applied:</span> {viewing.createdAt ? new Date(viewing.createdAt).toLocaleDateString() : 'ΓÇö'}</div>
            </div>
          </div>
        </div>
      )}
