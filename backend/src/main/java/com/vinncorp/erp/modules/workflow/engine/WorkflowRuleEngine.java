    ...
    private boolean evaluateRule(JSONObject rule, Task task, User user) {
        ...
        if (rule.has("requiredPermissions")) {
            ...
            for (int i = 0; i < perms.length(); i++) {
                ...
                boolean hasPerm = projectMemberRepository.hasPermission(task.getProject().getId(), user.getId(), permName);
                if (hasPerm) {
                    return true;
                }
            }
        ...
        if (rule.has("role")) {
            ...
            return projectMemberRepository.findByProject_IdAndUser_Id(task.getProject().getId(), user.getId())
