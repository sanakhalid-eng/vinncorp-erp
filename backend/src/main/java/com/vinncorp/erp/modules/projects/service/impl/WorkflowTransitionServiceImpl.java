...
public class WorkflowTransitionServiceImpl implements WorkflowTransitionService {
    ...
    @Override
    public WorkflowTransitionResponse createTransition(Long projectId,
                                                       WorkflowTransitionRequest request) {

        Project project = projectRepository.findById(projectId)
                .orElseThrow(() -> new ResourceNotFoundException("Project not found"));

        WorkflowStatus from = statusRepository.findById(request.getFromStatusId())
                .orElseThrow(() -> new ResourceNotFoundException("From status not found"));

        WorkflowStatus to = statusRepository.findById(request.getToStatusId())
                .orElseThrow(() -> new ResourceNotFoundException("To status not found"));
    ...
    public void validateTransition(Long projectId, Long fromId, Long toId) {
        ...
        WorkflowStatus from = statusRepository.findById(fromId)
                .orElseThrow(() -> new ResourceNotFoundException("Invalid from status"));

        WorkflowStatus to = statusRepository.findById(toId)
                .orElseThrow(() -> new ResourceNotFoundException("Invalid to status"));
    ...
    private void validateTransition(Long projectId, WorkflowStatus from, WorkflowStatus to) {
        ...
        boolean exists = transitionRepository
                .existsByProjectIdAndFromStatusIdAndToStatusId(projectId, from.getId(), to.getId());
