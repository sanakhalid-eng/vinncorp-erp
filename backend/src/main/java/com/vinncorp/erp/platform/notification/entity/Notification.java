...
@Index(name = "idx_notification_user_id", columnList = "user_id"), @Index(name = "idx_notification_is_read", columnList = "is_read"), @Index(name = "idx_notification_user_read", columnList = "user_id, is_read"), @Index(name = "idx_notification_dedup", columnList = "user_id, type, entity_id"), @Index(name = "idx_notification_expires", columnList = "expires_at")}) @Data

public class Notification extends BaseAuditableEntity {
@Id @GeneratedValue(strategy = GenerationType.IDENTITY) private Long id;
@Column(name = "event_id", unique = true, length = 100) private String eventId;
...
@Enumerated(EnumType.STRING) @Column(name = "category") private NotificationCategory category;
...
@Column(name = "project_name", length = 200) private String projectName;
...
@Column(name = "priority") @Enumerated(EnumType.STRING) private Priority priority = Priority.MEDIUM;
@Column(name = "expires_at") private LocalDateTime expiresAt;
@Column(name = "group_key", length = 200) private String groupKey;
@Column(name = "channel") private String channel = "IN_APP";
@ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "workspace_id", updatable = false) @NotFound(action = NotFoundAction.IGNORE) private Workspace workspace;
@Column(name = "workspace_id", insertable = false, updatable = false) private Long workspaceId;
